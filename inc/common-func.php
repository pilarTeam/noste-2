...
add_action( 'wp_ajax_update_status_for_edit_form', 'update_status_for_edit_form' );
...
function noste_update_project_step() {
	...
	/* Step Status */
		...
		$project_tmin_status[$form_id]['status'] = 0;

		if ( array_intersect( [ 'administrator', 'um_valvoja'], $user->roles ) ) {
			$project_tmin_status[$form_id]['status'] = 3;
		} elseif ( array_intersect( [ 'um_project-manager' ], $user->roles ) ) {
...
function noste_form_footer($type = 'form') {
	...
	if ( isset($project_tmin_status[$_GET['tmin']]) && !empty($project_tmin_status[$_GET['tmin']]) && isset($user->roles) && !empty($user->roles) && array_intersect( [ 'administrator' ], $user->roles ) ) {
		$preview_status = false;
		$roles = 'admin';
	} 
	...
	if ( isset($user->roles) && !empty($user->roles) && array_intersect( [ 'um_valvoja' ], $user->roles ) && $project_tmin_status[$_GET['tmin']]['status'] == 2 ) {
		$preview_status = true;
	} elseif ( isset($user->roles) && !empty($user->roles) && array_intersect( [ 'um_valvoja' ], $user->roles ) && $project_tmin_status[$_GET['tmin']]['status'] == 3 ) {
		$preview_status = false;
		$edit_prview = true;
	...
	if ( isset($user->roles) && !empty($user->roles) && array_intersect( [ 'um_project-manager' ], $user->roles ) && $project_tmin_status[$_GET['tmin']]['status'] == 3 ) {
		$edit_prview = true;
		$roles = 'manager';
	} elseif ( isset($user->roles) && !empty($user->roles) && array_intersect( [ 'um_project-manager' ], $user->roles ) && $project_tmin_status[$_GET['tmin']]['status'] == 2 ) {
		return;
	} elseif ( isset($user->roles) && !empty($user->roles) && array_intersect( [ 'um_project-manager' ], $user->roles ) && $project_tmin_status[$_GET['tmin']]['status'] != 2 ) {
		$edit_prview = false;
		$roles = 'manager';
	...
	?>
	    ...
	    <div class="popup_wrap hidden fixed left-0 top-0 w-full h-full bg-[#00151F66] z-50">
	        <div class="w-[95%] lg:w-auto rounded-[12px] bg-white border border-solid border-[#E1E1EA] max-w-[600px] absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]">
	            ...
	            <div class="p-4 flex gap-4 justify-end">
	                ...
	                <a href="<?php echo esc_attr( $edit_url ); ?>" data-tm="<?php echo esc_attr( $_GET['tm'] ?? '' ); ?>" data-tmin="<?php echo esc_attr( $_GET['tmin'] ?? '' ); ?>" data-post_id="<?php echo esc_attr( get_the_ID() ); ?>" class="submit_popup_form inline-block bg-[#00B2A9] rounded-lg px-3 lg:px-[10px] py-[5px] text-[14px] text-white">Kyllä, haluan muokata</a>
	            </div>
	        </div>
	    </div>
	    ...
	    <?php if ( $preview_status ): ?>
	    	...
	    	<?php else: ?>
	    		...
	    		<div class="card_footer <?php echo esc_attr( $edit_prview ? 'edit_form_wrap' : 'normal' ); ?> <?php echo esc_attr( $roles ); ?>" >
		            <div class="p-4 border-t border-line flex items-center justify-between pt-[20px] on-submit-btn">
		                <div>
		                ...
		                <div>
		                    <a href="" class="edit_form btn gap-2 border border-line bg-white">
					...
					<!-- Card footer -->
					<div class="p-4 border-t border-line without-submit">
			            <div class="flex items-center justify-between">
			                <a href="<?php echo esc_attr( site_url( remove_query_arg(['tmin']) ) ); ?>" class="btn gap-2 border border-line">
			                	<i class="um-faicon-angle-left"></i>
			                    Takaisin
...
add_action( 'wp_ajax_form_preview_from_twig', 'form_preview_from_twig');
function form_preview_from_twig() {

    if ( empty($_POST['tm']) || empty($_POST['tmin']) || empty($_POST['post_id']) ) {
		$error = new WP_Error( '001', "Not Found Proper Information" );
		wp_send_json_error( $error );
	}

    $response = (object) ['template' => false, 'submission' => [], 'global_keywords' => [], 'status' => 0];
    $ptname = implode('_', ['noste', $_POST['tm'], $_POST['tmin']]);

    $global_keywords = (array) get_post_meta((int) $_POST['post_id']);

    foreach ($global_keywords as $key => $value) {
        if ( count($value) == 1 ) {
        	$global_keywords[$key] = $value[0];
        }
    }
    ...
    $response->submission = (array) json_decode(get_post_meta((int) $_POST['post_id'], $ptname, true), true);

    $template = implode('/', (array) [$_POST['tm'], $_POST['tmin']]);
    $template_path = get_template_directory() . '/template-preview/' . $template . '.twig';

    $template_dir = get_template_directory() . '/template-preview/'. $_POST['tm'];
	...
	$project_tmin_status = !empty( get_post_meta( $_POST['post_id'], sprintf('%s_status', $_POST['tm']), true ) ) ? json_decode( get_post_meta( $_POST['post_id'], sprintf('%s_status', $_POST['tm']), true ), true ) : [];

	$response->status = $project_tmin_status[$_POST['tmin']]['status'] ?? 0;

	wp_send_json_success((array) $response, 200);
}


function update_status_for_edit_form() {
    if ( empty($_POST['tm']) || empty($_POST['tmin']) || empty($_POST['post_id']) ) {
		$error = new WP_Error( '001', "Not Found Proper Information" );
		wp_send_json_error( $error );
	}
	...
	$updated = update_post_meta( $post_id, sprintf('%s_status', $step_id), json_encode( $project_tmin_status ) );
